...
describe YotpoKafka::Consumer do
  ...
  it 'getting to consume message' do
    expect { Helpers::ConsumerHandler.new.handle_consume('message', Helpers::DummyMsg.new) }.to_not raise_error
  end

  it 'getting to consume message that raises error, key nil' do
    expect { Helpers::ConsumerHandlerWithError.new.handle_consume('message', Helpers::DummyMsg.new(key: nil)) }.to_not raise_error
  end

  it 'getting to consume message that raises error, key not nil' do
    expect { Helpers::ConsumerHandlerWithError.new.handle_consume('message', Helpers::DummyMsg.new(key: 'buya')) }.to_not raise_error
  end

  it 'getting to consume message that raises error, key not utf chars' do
    expect { Helpers::ConsumerHandlerWithError.new.handle_consume('message', Helpers::DummyMsg.new(key: 128.chr)) }.to_not raise_error
  end
  ...
  it 'handle error if consumer raises' do
    ...
    Helpers::ConsumerHandlerWithError.new(failures_topic: 'test.failures').handle_consume('message', Helpers::DummyMsg.new(key: 'buya'))
  end

  it 'sets failure headers if error' do
    message = Helpers::DummyMsg.new(key: 'buya')
